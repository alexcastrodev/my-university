...
export class JavaConceptsService {
  ...
  findBySlug(slug: string, lang: Language = DEFAULT_LANGUAGE): JavaConceptDetail | null {
  ...
  findAllDetailed(lang: Language = DEFAULT_LANGUAGE): JavaConceptDetail[] {
    const language = normalizeLanguage(lang);
  ...
  private readSummary(meta: ConceptMeta, lang: Language): JavaConceptSummary {
  ...
  private readDetail(meta: ConceptMeta, lang: Language): JavaConceptDetail {
    const availableLanguages = readAvailableLanguages(meta.slug);
    const { path, language } = this.resolveContentPath(meta.slug, lang);
