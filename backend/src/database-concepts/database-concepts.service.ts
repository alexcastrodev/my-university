...
export type DatabaseConceptCategory =
  | 'PostgreSQL'
  | 'SQL'
  | 'MongoDB'
  | 'DynamoDB'
...
export class DatabaseConceptsService {
  ...
  findAll(lang: Language = DEFAULT_LANGUAGE): DatabaseConceptSummary[] {
  ...
  findAllDetailed(lang: Language = DEFAULT_LANGUAGE): DatabaseConceptDetail[] {
    const language = normalizeLanguage(lang);
  ...
  private readDetail(meta: ConceptMeta, lang: Language): DatabaseConceptDetail {
    const {
      language,
      availableLanguages,
      body,
