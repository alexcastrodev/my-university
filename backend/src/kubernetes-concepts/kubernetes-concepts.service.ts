...
export interface KubernetesConceptDetail extends KubernetesConceptSummary {
  version: string | null;
...
const DATA_DIR = join(__dirname, '../seed/data/kubernetes-concepts');
...
export class KubernetesConceptsService {
  ...
  findAll(lang: Language = DEFAULT_LANGUAGE): KubernetesConceptSummary[] {
  ...
  findAllDetailed(lang: Language = DEFAULT_LANGUAGE): KubernetesConceptDetail[] {
    const language = normalizeLanguage(lang);
  ...
  private readDetail(meta: ConceptMeta, lang: Language): KubernetesConceptDetail {
    const {
      language,
      availableLanguages,
      body,
