...
- **Um RPO próximo de zero força replicação síncrona entre regiões** (ou um sistema construído sobre consenso entre regiões, como Spanner ou CockroachDB) — a única forma de garantir que nenhuma escrita commitada é perdida quando uma região desaparece é já ter colocado essa escrita de forma durável em outro lugar antes de confirmá-la. Isso te dá um RPO próximo de zero ao custo de a ida e volta entre regiões pousar na latência de cada escrita, que é o imposto de dezenas-a-centenas-de-milissegundos da Visão Geral, pago toda vez.
