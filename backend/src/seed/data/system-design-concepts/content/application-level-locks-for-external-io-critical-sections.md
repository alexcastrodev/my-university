...
[^advisory-lock-burst]: Worth expanding with real numbers: say the critical section makes two retried HTTP calls that take ~1.5s worst case, and the endpoint sees 50 concurrent requests during a burst. With the row-lease lock, that burst uses at most a handful of connections total — 49 of the 50 requests get `0` rows affected on the acquire `UPDATE` (a few milliseconds each) and return `409` immediately without ever touching the external API. With `pg_advisory_xact_lock`, the 49 losers don't get an instant rejection; they *block waiting on the lock itself*, each one holding a connection from the pool for the entire 1.5s while it waits — the same shape of problem as a fixed-size pool getting starved by a much smaller number of callers each holding a connection for far longer than the pool was sized for.
