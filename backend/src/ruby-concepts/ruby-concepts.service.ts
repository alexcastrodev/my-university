...
export class RubyConceptsService {
  ...
  findAll(lang: Language = DEFAULT_LANGUAGE): RubyConceptSummary[] {
  ...
  findAllDetailed(lang: Language = DEFAULT_LANGUAGE): RubyConceptDetail[] {
    const language = normalizeLanguage(lang);
    return this.conceptsMeta.map((meta) => this.readDetail(meta, language));
  }

  private readSummary(meta: ConceptMeta, lang: Language): RubyConceptSummary {
  ...
  private readDetail(meta: ConceptMeta, lang: Language): RubyConceptDetail {
    const {
      language,
      availableLanguages,
      body,
