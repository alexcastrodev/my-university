...
export class JvmConceptsController {
  constructor(
    private service: JvmConceptsService,
  ...
  async findAll(@OptionalUserId() userId: number | null, @Query('lang') lang?: string) {
    ...
    return concepts.map((concept) => ({
      ...concept,
      read: readSlugs.has(`jvm:${concept.slug}`),
    }));
  }
  ...
  async findOne(
    ...
    const read =
      userId !== null &&
      (await this.xp.hasEntry(userId, 'concept-read', `jvm:${slug}`));
    return { ...concept, read };
  }
  ...
  async markRead(@Param('slug') slug: string, @CurrentUserId() userId: number) {
    ...
    if (!concept) throw new NotFoundException();
    await this.xp.grantConceptReadXp(userId, `jvm:${slug}`);
    return { read: true };
  }
}
