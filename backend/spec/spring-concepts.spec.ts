...
describe('GET /spring-concepts', () => {
  it('returns a list of concept summaries', async () => {
    const res = await get('/spring-concepts');
  ...
  it('summaries expose slug, id, category, title, summary and publishedAt but no sections', async () => {
    const body = await json<any[]>(await get('/spring-concepts'));
    const concept = body.find(
      (c) => c.slug === 'spring-boot-autoconfiguration',
    );
...
describe('GET /spring-concepts/:slug', () => {
  it('returns the full concept for a known slug', async () => {
    const res = await get('/spring-concepts/spring-boot-autoconfiguration');
    expect(res.status).toBe(200);
    const body = await json<any>(res);
    expect(body.slug).toBe('spring-boot-autoconfiguration');
    expect(body.category).toBe('Spring Boot');
  ...
  it('splits the markdown body into the fixed concept sections', async () => {
    const body = await json<any>(
      await get('/spring-concepts/spring-boot-autoconfiguration'),
    );
  ...
  it('returns 404 for an unknown slug', async () => {
    const res = await get('/spring-concepts/does-not-exist');
    expect(res.status).toBe(404);
  });

  it('returns 404 for a path-traversal slug', async () => {
    const res = await get('/spring-concepts/..%2f..%2fetc%2fpasswd');
    expect(res.status).toBe(404);
  });
});

describe('PUT /spring-concepts/:slug/read', () => {
  it('returns 404 for an unknown slug', async () => {
    const { cookie } = await login(`spring-concept-404-${Date.now()}`);
    const res = await put(
      '/spring-concepts/does-not-exist/read',
      {},
      { Cookie: cookie },
    );
    expect(res.status).toBe(404);
  ...
  it('returns 401 when no session cookie is present', async () => {
    const res = await put(
      '/spring-concepts/spring-boot-autoconfiguration/read',
      {},
      {},
    );
  ...
  it('grants XP once and is idempotent on repeated calls', async () => {
    const { cookie } = await login(`spring-concept-read-${Date.now()}`);
    const before = (
