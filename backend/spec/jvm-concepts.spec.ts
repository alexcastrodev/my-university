...
describe('GET /jvm-concepts', () => {
  it('returns a list of concept summaries', async () => {
    const res = await get('/jvm-concepts');
  ...
  it('summaries expose slug, id, title, summary and publishedAt but no sections', async () => {
    const body = await json<any[]>(await get('/jvm-concepts'));
    const concept = body.find((c) => c.slug === 'class-file-structure');
...
describe('GET /jvm-concepts/:slug', () => {
  it('returns the full concept for a known slug', async () => {
    const res = await get('/jvm-concepts/class-file-structure');
    expect(res.status).toBe(200);
    const body = await json<any>(res);
  ...
  it('splits the markdown body into the fixed concept sections', async () => {
    const body = await json<any>(
      await get('/jvm-concepts/class-file-structure'),
    );
  ...
  it('returns 404 for an unknown slug', async () => {
    const res = await get('/jvm-concepts/does-not-exist');
    expect(res.status).toBe(404);
  });

  it('returns 404 for a path-traversal slug', async () => {
    const res = await get('/jvm-concepts/..%2f..%2fetc%2fpasswd');
    expect(res.status).toBe(404);
  });
});

describe('PUT /jvm-concepts/:slug/read', () => {
  it('returns 404 for an unknown slug', async () => {
    const { cookie } = await login(`jvm-concept-404-${Date.now()}`);
    const res = await put(
      '/jvm-concepts/does-not-exist/read',
      {},
      { Cookie: cookie },
    );
    expect(res.status).toBe(404);
  ...
  it('returns 401 when no session cookie is present', async () => {
    const res = await put('/jvm-concepts/class-file-structure/read', {}, {});
    expect(res.status).toBe(401);
  });

  it('grants XP once and is idempotent on repeated calls', async () => {
    const { cookie } = await login(`jvm-concept-read-${Date.now()}`);
    const before = (
      await json<{ total: number }>(await get('/xp', { Cookie: cookie }))
    ).total;

    const res1 = await put(
      '/jvm-concepts/class-file-structure/read',
      {},
      { Cookie: cookie },
    );
