...
describe('GET /ruby-concepts', () => {
  it('returns a list of concept summaries', async () => {
    const res = await get('/ruby-concepts');
  ...
  it('summaries expose slug, id, title, topic, summary and publishedAt but no sections', async () => {
    const body = await json<any[]>(await get('/ruby-concepts'));
    const concept = body.find((c) => c.slug === 'object-model-and-method-lookup');
...
describe('GET /ruby-concepts/:slug', () => {
  it('returns the full concept for a known slug', async () => {
    const res = await get('/ruby-concepts/object-model-and-method-lookup');
    expect(res.status).toBe(200);
    const body = await json<any>(res);
    expect(body.slug).toBe('object-model-and-method-lookup');
  ...
  it('splits the markdown body into the fixed concept sections', async () => {
    const body = await json<any>(
      await get('/ruby-concepts/object-model-and-method-lookup'),
    );
  ...
  it('returns 404 for an unknown slug', async () => {
    const res = await get('/ruby-concepts/does-not-exist');
    expect(res.status).toBe(404);
  });

  it('returns 404 for a path-traversal slug', async () => {
    const res = await get('/ruby-concepts/..%2f..%2fetc%2fpasswd');
    expect(res.status).toBe(404);
  });
});

describe('PUT /ruby-concepts/:slug/read', () => {
  it('returns 404 for an unknown slug', async () => {
    const { cookie } = await login(`ruby-concept-404-${Date.now()}`);
    const res = await put(
      '/ruby-concepts/does-not-exist/read',
      {},
      { Cookie: cookie },
    );
    expect(res.status).toBe(404);
  ...
  it('returns 401 when no session cookie is present', async () => {
    const res = await put(
      '/ruby-concepts/object-model-and-method-lookup/read',
      {},
      {},
    );
  ...
  it('grants XP once and is idempotent on repeated calls', async () => {
    const { cookie } = await login(`ruby-concept-read-${Date.now()}`);
    const before = (
