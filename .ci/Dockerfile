...
COPY backend/ ./
RUN pnpm run build
# The runtime stage copies node_modules as-is, so drop the compiler/test/lint toolchain first.
RUN pnpm prune --prod
...
# tini: PID 1 that forwards stop signals and reaps zombies (see .ci/container-start.sh).
...
COPY .ci/container-start.sh /usr/local/bin/container-start
